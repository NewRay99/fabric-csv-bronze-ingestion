...
NOTEBOOK = ROOT / "99_common_library.py"
...
def main():
    ...
    print("PASS common-library Fabric source and code-cell syntax")
    ...
    shared_callers = {
        "00_archive_load.py": {"qident", "append_rows"},
        "01a_cfg_schema_capture_archive.py": {"qident", "append_rows"},
        "02_silver_formatter.py": {"drift_event_row"},
        "02a_archive_silver.py": {"drift_event_row"},
        "90_run_live_pipeline.py": {"merge_monitor_row"},
        "90_run_archive_pipeline.py": {"merge_monitor_row"},
    }
    for notebook_name, function_names in shared_callers.items():
        child = read_notebook(ROOT / notebook_name)
        child_source = "\n".join(
