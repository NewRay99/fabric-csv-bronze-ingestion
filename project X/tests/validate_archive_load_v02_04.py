...
def cell_containing(marker):
    matches = [value for value in cell_sources if marker in value]
    assert len(matches) == 1, f"Expected one cell containing {marker!r}; found {len(matches)}"
    return matches[0]


parameters = cell_containing("STOP_ON_FIRST_ERROR = False")
helpers = cell_containing("def discover_archive_files_dataframe(")
...
assert "null_primary_key_count BIGINT" in setup_source
