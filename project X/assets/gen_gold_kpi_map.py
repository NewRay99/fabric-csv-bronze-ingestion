...
M = {
    ...
    73: ("`fact_ipa`", "IPA Exists (row helper)", "C"),
    ...
    79: ("`fact_offer` ⟕ `fact_ipa`", "Offers Awaiting IPA Creation", "C"),
    80: ("—", "Blocked — no IPA-grain signature status (see KPI-77)", "B"),
    81: ("`fact_offer` ⟕ `fact_ipa`", "Is Awaiting IPA Creation (row helper)", "C"),
    82: ("—", "Blocked — no IPA-grain signature status (see KPI-77)", "B"),
    83: ("`fact_offer` ⟕ `fact_ipa`", "Accepted Offer to IPA Conversion %", "C"),
    84: ("`fact_offer` ⟕ `fact_ipa`", "Offers Still to Progress to IPA %", "C"),
    ...
    107: ("`fact_ipa[estimated_weekly_cost]`", "Estimated Active Weekly Cost — estimate, not signed-only actuals", "X"),
