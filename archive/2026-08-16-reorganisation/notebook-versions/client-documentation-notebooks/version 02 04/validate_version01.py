...
assert "F.row_number().over(window)" in archive_source
...
assert "Primary-key sample" in archive_source
assert "Silver export_date" in archive_source
...
with (ROOT / "schema_definition.csv").open(encoding="utf-8-sig", newline="") as handle:
